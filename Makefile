...
package-deb-amd64: build-linux-amd64
	@mkdir -p $(DIST_DIR)
	VERSION=$(PKG_VERSION) ARCH=amd64 envsubst < nfpm.yaml > nfpm-substituted.yaml
	nfpm package -p deb -f nfpm-substituted.yaml -t $(DIST_DIR)/

package-deb-arm64: build-linux-arm64
	@mkdir -p $(DIST_DIR)
	VERSION=$(PKG_VERSION) ARCH=arm64 envsubst < nfpm.yaml > nfpm-substituted.yaml
	nfpm package -p deb -f nfpm-substituted.yaml -t $(DIST_DIR)/

package-rpm-amd64: build-linux-amd64
	@mkdir -p $(DIST_DIR)
	VERSION=$(PKG_VERSION) ARCH=amd64 envsubst < nfpm.yaml > nfpm-substituted.yaml
	nfpm package -p rpm -f nfpm-substituted.yaml -t $(DIST_DIR)/
